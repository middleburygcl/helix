...
void read_polydata(int64_t fid,
                   GmfKwdCod header_type,
                   GmfKwdCod entity_type,
  ...
  for (int64_t k = 0; k < n_poly; k++) {
    ...
    entity.resize(v - u);
    for (index_t m = u; m < v; m++)
      entity[m - u] = entities[m] - 1;
...
void write(const Mesh& mesh, const std::string& filename) {
  ...
  // header
  fprintf(out, "# obj mesh from helix\n");
